...
export function WarehouseSidebar({
  ...
  navItems: { label: string; href: string; icon: React.ReactNode; badge?: number }[];
  sessionName: string;
  orgName: string;
}) {
    ...
    >
      <div className={`px-4 py-8 flex flex-col items-center gap-1 ${expanded ? "items-start px-8" : ""}`}>
        {expanded ? (
          <>
            ...
            <span className="text-[9px] uppercase tracking-[0.2em] font-bold text-[#8c7849]">Warehouse</span>
          </>
        ) : (
      ...
      {expanded && (
        <div className="px-8 mb-8 mt-auto">
          <button className="w-full py-3 bg-white text-black font-bold text-xs uppercase tracking-widest hover:bg-gray-200 transition-colors">
            New Order
          </button>
        </div>
      )}
